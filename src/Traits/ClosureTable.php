...
trait ClosureTable
{
    ...
    public static function init()
    {
        ...
        static::event('after_update', function ($model) {
            \think\Log::record('after_updateafter_updateafter_updateafter_updateafter_update');

            $model->updateClosure();
        });

        static::event('after_insert', function ($model) {
            \think\Log::record('after_insertafter_insertafter_insertafter_insertafter_insertafter_insertafter_insert');
            $model->insertClosure($model->getParentKey() ?: 0);
        });

        static::event('after_delete', function ($model) {
            \think\Log::record('after_deleteafter_deleteafter_deleteafter_deleteafter_delete');
    ...
    {
        return $this->getPk()?true:false;
    }

    public function getName()
    {
        return config('database.prefix').$this->name;
    }
    /**
    ...
    {
        $key = $this->getKey();
        $table = $this->getPrefixedClosureTable();
        $ancestorColumn = $this->getAncestorColumn();
        $descendantColumn = $this->getDescendantColumn();
    ...
    public function createSibling(array $attributes)
    {
        ...
        $model = null;
        $parentKey = $this->getParents()->getKey();
        $attributes[$this->getParentColumn()] = $parentKey;
        $model = $this->forceCreate($attributes);
        return $model;
    }
    ...
    {
        return $this->queryBesides()->select();
    }
    ...
    {
        return $this->queryChildren()->select();
    }
